...
async def post_credentials(
    ...
    **Raises:**
    ...
    logger.debug(f"POST credentials body: {credentials.dict()}")
    ...
    async with httpx.AsyncClient() as client:
        ...
        logger.info(
            f"GET versions status_code: {response_versions.status_code}"
        )
        ...
        if response_versions.status_code == fastapistatus.HTTP_200_OK:
            ...
            if not version_url:
                logger.debug(
                    f"Version {VersionNumber.v_2_2_1} is not supported"
                )
            ...
            if response_endpoints.status_code == fastapistatus.HTTP_200_OK:
                ...
                logger.debug(
                    f"GET version details response data: {endpoints}"
                )
...
async def update_credentials(
    ...
    **Raises:**
    ...
    logger.debug(f"PUT credentials body: {credentials.dict()}")
    auth_token = get_auth_token(request)
